...
#HPA & PBS
Idents(HPA_PBS_merged) <- "seurat_clusters"
HPA_PBS_ConditionMarkers <- FindAllMarkers(HPA_PBS_merged, only.pos = T)
HPA_PBS_ConditionMarkers_top50 <- HPA_PBS_ConditionMarkers %>%
  group_by(cluster) %>% 
  slice_max(n = 50, avg_log2FC)
write.csv(HPA_PBS_ConditionMarkers_top50, "HPA_PBS_ConditionMarkers_top50.csv")
write.csv(HPA_PBS_ConditionMarkers, "HPA_PBS_ConditionMarkers.csv")
AvgExp_HPS_PBS <- AverageExpression(HPA_PBS_merged)
write.csv(AvgExp_HPS_PBS$SCT, "AvgExp_HPS_PBS.csv")
...
neuron_genes <- c("Meis2", "Myt1l", "Mylk", "Bcl11b",
                  ...
                  "Pou3f1", "Sp9", "Ebf1", "Gatad1", "Trank1", "Erbb4",
                  "Adarb2", "Pde4d")
...
ggsave("PBS_HPA_CellType_Plot_res1.2_dims40.jpeg", device = "jpeg", 
