...
plot1 + plot2 + plot3
gem2_seurat_object <- subset(gem2_seurat_object, subset = nFeature_RNA > 200 & nFeature_RNA < 500 & percent.mt < 15)
...
ElbowPlot(object = gem2_seurat_object, ndims = 20)
gem2_seurat_object <- FindNeighbors(object = gem2_seurat_object, dims = 1:15)
gem2_seurat_object <- FindClusters(object = gem2_seurat_object, resolution = 0.7)
##
gem2_seurat_object <- RunUMAP(gem2_seurat_object, dims = 1:15)
DimPlot_scCustom(gem2_seurat_object, label = T, split.by = "condition")

sweep.res.list_gem2 <- paramSweep_v3(gem2_seurat_object, PCs = 1:15, sct = T, num.cores = 2)
...
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
gem2_seurat_object <- doubletFinder_v3(gem2_seurat_object, PCs = 1:15, pN = 0.25, pK = 0.11, nExp = nExp_poi, reuse.pANN = FALSE, sct = T)
gem2_seurat_object@meta.data[,"CellTypes_DF"] <- gem2_seurat_object$DF.classifications_0.25_0.11_293
gem2_seurat_object@meta.data$CellTypes_DF[which(gem2_seurat_object$pANN_0.25_0.11_293 == "Doublet")] <- "Doublet"
...
plot1 + plot2 + plot3
gem3_seurat_object <- subset(gem3_seurat_object, subset = nFeature_RNA > 200 & nFeature_RNA < 500 & percent.mt < 15)
...
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
gem3_seurat_object <- doubletFinder_v3(gem3_seurat_object, PCs = 1:10, pN = 0.25, pK = 0.22, nExp = nExp_poi, reuse.pANN = FALSE, sct = T)
gem3_seurat_object@meta.data[,"CellTypes_DF"] <- gem3_seurat_object$DF.classifications_0.25_0.22_1040
gem3_seurat_object@meta.data$CellTypes_DF[which(gem3_seurat_object$pANN_0.25_0.22_1040 == "Doublet")] <- "Doublet"
...
plot1 + plot2 + plot3
gem4_seurat_object <- subset(gem4_seurat_object, subset = nFeature_RNA > 200 & nFeature_RNA < 500 & percent.mt < 10)
...
gem4_seurat_object <- FindNeighbors(object = gem4_seurat_object, dims = 1:10)
gem4_seurat_object <- FindClusters(object = gem4_seurat_object, resolution = 1.2)
gem4_seurat_object <- RunUMAP(gem4_seurat_object, dims = 1:10)
clust <- DimPlot_scCustom(gem4_seurat_object, label = T, split.by = "condition")

sweep.res.list_gem4 <- paramSweep_v3(gem4_seurat_object, PCs = 1:10, sct = T, num.cores = 2)
...
nExp_poi <- round(0.08*nrow(gem4_seurat_object@meta.data)) #tailor for your dataset
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
gem4_seurat_object <- doubletFinder_v3(gem4_seurat_object, PCs = 1:10, pN = 0.25, pK = 0.12, nExp = nExp_poi, reuse.pANN = FALSE, sct = T)
gem4_seurat_object@meta.data[,"CellTypes_DF"] <- gem4_seurat_object$DF.classifications_0.25_0.12_1502
gem4_seurat_object@meta.data$CellTypes_DF[which(gem4_seurat_object$pANN_0.25_0.12_1502 == "Doublet")] <- "Doublet"
doublet <- DimPlot(gem4_seurat_object, group.by="CellTypes_DF", reduction="umap", pt.size=0.5, order=c("Coll.Duct.TC","Doublet"), cols=c("#66C2A5","#FFD92F","#8DA0CB","#A6D854","#E78AC3","#B3B3B3","#E5C494","black","#FC8D62"))
...
tibble(
  ...
  Dataset = gem4_seurat_object$DF.classifications_0.25_0.12_1502
) %>%
...
gem5_seurat_object <- FindClusters(object = gem5_seurat_object, resolution = 2.0)
...
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
gem5_seurat_object <- doubletFinder_v3(gem5_seurat_object, PCs = 1:10, pN = 0.25, pK = 0.15, nExp = nExp_poi, reuse.pANN = FALSE, sct = T)
gem5_seurat_object@meta.data[,"CellTypes_DF"] <- gem5_seurat_object$DF.classifications_0.25_0.15_5429
gem5_seurat_object@meta.data$CellTypes_DF[which(gem5_seurat_object$pANN_0.25_0.15_5429 == "Doublet")] <- "Doublet"
...
tibble(
  ...
  Dataset = gem5_seurat_object$DF.classifications_0.25_0.15_5429
) %>%
  group_by(cluster, Dataset) %>%
  count() %>%
  group_by(cluster) %>%
...
ElbowPlot(object = gem6_seurat_object, ndims = 20)
gem6_seurat_object <- FindNeighbors(object = gem6_seurat_object, dims = 1:15)
gem6_seurat_object <- FindClusters(object = gem6_seurat_object, resolution = 0.7)
gem6_seurat_object <- RunUMAP(gem6_seurat_object, dims = 1:15)
DimPlot_scCustom(gem6_seurat_object, label = T, split.by = "condition")

sweep.res.list_gem6 <- paramSweep_v3(gem6_seurat_object, PCs = 1:15, sct = T, num.cores = 2)
...
nExp_poi <- round(0.02*nrow(gem6_seurat_object@meta.data)) #tailor for your dataset
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
gem6_seurat_object <- doubletFinder_v3(gem6_seurat_object, PCs = 1:15, pN = 0.25, pK = 0.3, nExp = nExp_poi, reuse.pANN = FALSE, sct = T)
gem6_seurat_object@meta.data[,"CellTypes_DF"] <- gem6_seurat_object$DF.classifications_0.25_0.3_869
gem6_seurat_object@meta.data$CellTypes_DF[which(gem6_seurat_object$pANN_0.25_0.3_869 == "Doublet")] <- "Doublet"
...
plot1 + plot2 + plot3
gem7_seurat_object <- subset(gem7_seurat_object, subset = nFeature_RNA > 200 & nFeature_RNA < 750 & percent.mt < 10)
...
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
gem7_seurat_object <- doubletFinder_v3(gem7_seurat_object, PCs = 1:15, pN = 0.25, pK = 0.13, nExp = nExp_poi, reuse.pANN = FALSE, sct = T)
gem7_seurat_object@meta.data[,"CellTypes_DF"] <- gem7_seurat_object$DF.classifications_0.25_0.13_468
gem7_seurat_object@meta.data$CellTypes_DF[which(gem7_seurat_object$pANN_0.25_0.13_468 == "Doublet")] <- "Doublet"
...
gem8_seurat_object <- FindNeighbors(object = gem8_seurat_object, dims = 1:10)
gem8_seurat_object <- FindClusters(object = gem8_seurat_object, resolution = 1.2)
gem8_seurat_object <- RunUMAP(gem8_seurat_object, dims = 1:10)
DimPlot_scCustom(gem8_seurat_object, label = T, split.by = "condition")

sweep.res.list_gem8 <- paramSweep_v3(gem8_seurat_object, PCs = 1:10, sct = T, num.cores = 2)
...
nExp_poi <- round(0.15*nrow(gem8_seurat_object@meta.data)) #tailor for your dataset
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
gem8_seurat_object <- doubletFinder_v3(gem8_seurat_object, PCs = 1:15, pN = 0.25, pK = 0.07, nExp = nExp_poi, reuse.pANN = FALSE, sct = T)
gem8_seurat_object@meta.data[,"CellTypes_DF"] <- gem8_seurat_object$DF.classifications_0.25_0.07_1728
gem8_seurat_object@meta.data$CellTypes_DF[which(gem8_seurat_object$pANN_0.25_0.07_1728 == "Doublet")] <- "Doublet"
...
ElbowPlot(object = gem9_seurat_object, ndims = 20)
...
nExp_poi <- round(0.25*nrow(gem9_seurat_object@meta.data)) #tailor for your dataset
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
gem9_seurat_object <- doubletFinder_v3(gem9_seurat_object, PCs = 1:15, pN = 0.25, pK = 0.1, nExp = nExp_poi, reuse.pANN = FALSE, sct = T)
gem9_seurat_object@meta.data[,"CellTypes_DF"] <- gem9_seurat_object$DF.classifications_0.25_0.1_12096
gem9_seurat_object@meta.data$CellTypes_DF[which(gem9_seurat_object$pANN_0.25_0.1_12096 == "Doublet")] <- "Doublet"
...
tibble(
  ...
  Dataset = gem9_seurat_object$DF.classifications_0.25_0.1_12096
) %>%
  group_by(cluster, Dataset) %>%
  count() %>%
  group_by(cluster) %>%
...
gem10_seurat_object <- FindClusters(object = gem10_seurat_object, resolution = 1.5)
...
nExp_poi.adj <- round(nExp_poi*(1-homotypic.prop))
gem10_seurat_object <- doubletFinder_v3(gem10_seurat_object, PCs = 1:15, pN = 0.25, pK = 0.04, nExp = nExp_poi, reuse.pANN = FALSE, sct = T)
gem10_seurat_object@meta.data[,"CellTypes_DF"] <- gem10_seurat_object$DF.classifications_0.25_0.04_963
gem10_seurat_object@meta.data$CellTypes_DF[which(gem10_seurat_object$pANN_0.25_0.04_963 == "Doublet")] <- "Doublet"
